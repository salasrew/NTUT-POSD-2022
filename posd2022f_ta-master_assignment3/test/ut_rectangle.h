...
TEST(recTest,createIterator)
{
    ...
    Iterator *temp = rec.createIterator(new BFSIteratorFactory());
...
TEST(recTest,getPoints)
{
    ...
    ASSERT_TRUE(rec.getPoints().size()==4);
}
TEST(recTest,accept)
{
    Point point1(-2, 1);
    ...
    EXPECT_NO_THROW(rec.accept(&recDetector));
}
